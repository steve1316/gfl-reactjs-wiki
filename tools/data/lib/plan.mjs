 ...
 * committed shards. The collaboration dolls added by hand through `overrides.json` `addDolls` are left out of the comparison unless
 * upstream releases one of them itself.
 *
...
export function planRefresh({ lockedSha, latestSha, releasedIds, committedIds, extraIds }) {
	...
	const committed = new Set(committedIds.filter((id) => !extras.has(id) || released.has(id)));
