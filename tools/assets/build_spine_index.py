...
`--v3` indexes the skin-id layout the asset rebuild stages instead: `spine/<id>/` holds only the base combat and dorm rigs, `mod/` the Mod
rigs and `skins/<skinId>/` each skin's rigs. Every rig path in that index is relative to `spine/<id>/`, and `anims` is left empty for
`add_spine_animations.mjs` to fill in.
"""
